...
<v-app>
    <v-toolbar color="red lighten-1" app>
        ...
        <v-spacer></v-spacer>
        <v-text-field append-icon="search" v-model="searchVideoSync" label="Oylamak için şarkı arayın..." solo class="mt-2"></v-text-field>
        <v-spacer></v-spacer>
        <v-tooltip bottom>
        ...
        <v-layout fill-height>
            <v-dialog v-model="isAdding" max-width="850px">
                <v-card>
                    ...
                    <v-card-text>
                        <v-autocomplete v-model="searchModelText" :items="searchItems" :loading="searchLoading" :search-input.sync="searchModelTextSync" small-chips cache-items hide-no-data item-text="name" item-value="video_id" label="Search music..." solo>
            ...
            <v-flex xs12 class="ma-2">
                <v-data-table :items="getItems()" class="elevation-1" hide-actions hide-headers v-if="!songLoading">
                    <template slot="items" slot-scope="props">
                        ...
                        <td class="text-xs-right">{{ props.item.realaseDate }}</td>
                        <td class="text-xs-right">{{ props.item.signer }}</td>
    ...
    methods: {
        ...
        async saveMusic() {
            if (this.searchModelText != "") {
                this.isAdding = false;
                const VIDEO_API_LINK = 'https://www.googleapis.com/youtube/v3/videos?id=' + this.searchModelText + '&part=snippet&key=' + this.API_KEY;
        ...
        playVideo(id) {
            let embed_key = "http://www.youtube.com/embed/" + id + "?autoplay=1&origin=http://localhost:8080";
        ...
        watch: {
            searchModelTextSync(val) {
                if (val != null || val != '') {
                    ...
                    const YOUTUBE_LINK = 'https://www.googleapis.com/youtube/v3/search?q=' + val + '&maxResults=50&part=snippet&key=' + this.API_KEY + '&type=regionCode:90';
                    let responseItems = [];
                    fetch(YOUTUBE_LINK)
                        .then(res => res.json())
                        .then((data) => {
